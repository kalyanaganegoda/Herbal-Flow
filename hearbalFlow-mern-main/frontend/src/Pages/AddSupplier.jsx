...
export const AddSupplier = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!/^[0-9]{9}$/.test(mobile)) {
      Swal.fire("Invalid Input", "Mobile number must be 9 digits.", "error");
      return;
    }
    try {
      const requestBody = {
      ...
      const response = await axios.post(
        "http://localhost:3000/api/supplier/addSupplier",
        requestBody
      );
      Swal.fire("Success", "New Supplier Successfully Added!", "success");
      navigate("/supplier");
    } catch (error) {
      console.log(error);
      Swal.fire("Error", "Error Adding New Supplier", "error");
    }
  };
        ...
        <div className="pl-16 pt-2 pr-10 mt-20">
          <form onSubmit={handleSubmit}>
            <div className="bg-slate-200 p-4 rounded-2xl shadow-sm">
              <h2 className="text-2xl font-bold mb-5">Supplier Information</h2>
              ...
              <div className="flex flex-col mb-4">
                <label className="block text-gray-700 required">Mobile:</label>
                <input
                  type="tel"
                  value={mobile}
